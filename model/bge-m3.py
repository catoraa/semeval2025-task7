...
# 读取任务文件
posts = pd.read_csv("../preprocess_data/posts_pre.csv").fillna('')
fact_checks = pd.read_csv("../preprocess_data/fact_checks_pre.csv").fillna('')
...
# 加载模型
model = SentenceTransformer("BAAI/bge-m3").bfloat16().to(device)
cross_encoder = CrossEncoder("BAAI/bge-m3")
...
for i, post_id in enumerate(predictions.index):
    ...
    top_indices = np.argsort(post_similarities)[-top_k:]  # 直接取Top K，不考虑阈值

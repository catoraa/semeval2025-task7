...
with open('../preprocess_data/trial/trial_predictions.json', 'r') as file:
    result = json.load(file)
with open('../preprocess_data/trial/tasks.json', 'r') as file:
...
fact_checks.set_index('fact_check_id')
...
for language, language_tasks in tasks.items():
    ...
    for post_dev_id in post_dev_ids:
        ...
        for fact_check_id in fact_checks_ids:
            score = similarities[predictions.index.get_loc(post_dev_id)][fact_checks.index.get_loc(fact_check_id)]
            scores.append((score, fact_check_id))
        scores = heapq.nlargest(top_k, scores, key=lambda item: item[0])
        for _, fact_check_id in scores:

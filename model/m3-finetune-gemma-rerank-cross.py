...
# 读取任务文件
posts = pd.read_csv("../preprocess_data/dev/cross/posts.csv").fillna('')
fact_checks = pd.read_csv("../preprocess_data/dev/cross/fact_checks.csv").fillna('')
pairs = pd.read_csv('../preprocess_data/dev/cross/pairs.csv').fillna('')
predictions = pd.read_json("../preprocess_data/dev/cross/crosslingual_predictions.json", orient='index')
print('数据读取完成!')
...
for i, post_id in enumerate(predictions.index):
    ...
    top_indices = np.argsort(post_similarities)[-pre_top_k:]
    top_k_fact_checks = [fact_check_ids[idx] for idx in top_indices]
    scores = reranker.compute_score([posts_sentences[i]] + top_k_fact_checks)
    ranked_indices = np.argsort(scores)[-top_k:]
    top_k_fact_check_indices = [top_indices[idx] for idx in ranked_indices]
    result[post_id] = top_k_fact_check_indices


result_converted = {post_id: [int(idx) for idx in indices] for post_id, indices in result.items()}

# 保存到 JSON 文件
with open("../output/dev/crosslingual_predictions.json", "w") as f:
